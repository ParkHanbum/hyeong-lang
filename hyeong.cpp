...
#include <llvm/IR/LLVMContext.h> // 추가
...
std::vector<Token> tokenize(const std::string &source) {
  ...
  while (pos < source.length()) {
    ...
    if (isHangul(c)) {
      ...
      inCommand = true;
      current += source.substr(pos - (c < 0x80 ? 1 : c < 0x800 ? 2
                                                               : 3),
                               c < 0x80 ? 1 : c < 0x800 ? 2
                                                        : 3);
      ...
      if (c == '?')
        ...
        tokens.push_back({std::string(1, c == 0x2665 ? 'H' : 'L'), Token::HEART}); // 임시로 H, L로 대체
    }
  }
  if (!current.empty()) {
    tokens.push_back({current, inCommand ? Token::COMMAND : Token::DOTS});
...
class HyeongCompiler {
  LLVMContext context; // 추가: 컨텍스트 멤버
  Module *module;
  IRBuilder<> builder;
...
public:
  HyeongCompiler() : builder(context) {
    ...
    Int32Ty = builder.getInt32Ty(); // 초기화
    // 수정: FunctionCallee에서 바로 Function* 추출
  ...
  void compile(const std::vector<ASTNode> &ast) {
    ...
    for (size_t i = 0; i < ast.size(); i++) {
      ...
      if (node.command.find("형") == 0 || (node.command.find("혀") == 0 && node.command.find("엉") != std::string::npos)) {
        int value = node.letterCount * node.dotCount;
        pushStack(currentStack, builder.getInt32(value));
      } else if (node.command.find("하") == 0 && node.command.find("앙") != std::string::npos) {
      ...
      } else if (node.command.find("하") == 0 && node.command.find("앗") != std::string::npos) {
        Value *prod = builder.getInt32(1);
        for (int j = 0; j < node.letterCount; j++) {
          prod = builder.CreateMul(prod, popStack(currentStack));
        }
        pushStack(node.dotCount, prod);
      } else if (node.command.find("흐") == 0 && node.command.find("읏") != std::string::npos) {
        ...
        pushStack(node.dotCount, sum);
      } else if (node.command.find("흐") == 0 && node.command.find("읍") != std::string::npos) {
        std::vector<Value *> values;
        for (int j = 0; j < node.letterCount; j++) {
          values.push_back(popStack(currentStack));
        }
        ...
        pushStack(node.dotCount, prod);
      } else if (node.command.find("흐") == 0 && node.command.find("윽") != std::string::npos) {
